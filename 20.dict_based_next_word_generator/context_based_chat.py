...
while True:
    ...
    final_result = ""
    if len(splits) > 1:
        final_result = splits[1]
        print(final_result)
    else:
        final_result = splits[0]
        print(final_result)

    global_string_corrector_dict = text_generator.get_global_string_word_based_corrector_dict_by_using_yingshaoxo_method(context, levels=2)
    new_response = text_generator.correct_sentence_based_on_word_by_using_yingshaoxo_method(final_result, global_string_corrector_dict=global_string_corrector_dict)
    print("\n**********\n", new_response,"\n************\n")
"""
