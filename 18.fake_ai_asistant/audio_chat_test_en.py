...
speech_recognizer = Speech_Recognizer(language="en")
yingshaoxo_translator = Yingshaoxo_Translator()
yingshaoxo_text_to_speech = Yingshaoxo_Text_to_Speech()

all_input_text = ""

yingshaoxo_text_to_speech.speak_it("Initialized.")
while True:
    trigger_get_touched = False
    while trigger_get_touched == False:
        trigger_text = speech_recognizer.recognize_following_speech()
        trigger_text = trigger_text.strip()
        if trigger_text in ["hi", "listen", "hello"] or "dear" in trigger_text.lower():
            trigger_get_touched = True
            yingshaoxo_text_to_speech.speak_it("What you want to say?")

    while True:
        en_text = speech_recognizer.recognize_following_speech(timeout_in_seconds=10)
        if (en_text.strip() == ""):
            #continue
            break

        yingshaoxo_text_to_speech.speak_it("Got it.")
        print(en_text)
        ...
        print(response)
        yingshaoxo_text_to_speech.speak_it("The End.")
        print("\n\n---------\n\n")