...
def create_train_env2(world, stage, actions, output_path=None):
...
class MultipleEnvironments:
    def __init__(self, world, stage, action_type, num_envs, output_path=None):
        ...
        self.envs = [create_train_env2(
            world, stage, actions, output_path=output_path) for _ in range(num_envs)]
    ...
    def run(self, index):
        ...
        while True:
            ...
            if request == "step":
                self.env_conns[index].send(
                    self.envs[index].step(action.item()))
            elif request == "reset":
