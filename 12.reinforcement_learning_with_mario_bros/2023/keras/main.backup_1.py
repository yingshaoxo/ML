...
class Trainer:
    ...
    max_storage_for_life_experience = 10
    ...
    def loop(self):
        ...
        while True:
            ...
            if random.randint(0, 3) == 0 or (not self.it_is_not_none(self.last_state)):
                ...
                if (str(result[1][0][0]).lower() == "nan"):
                    print("nan error...")
                    continue
                if (str(result[0][0][0]).lower() == "nan"):
                    print("nan error...")
                    continue
                action_probability = result[0]
                action_probability /= action_probability.sum()
                action = numpy.random.choice(self.the_action_numbers, p=numpy.squeeze(action_probability))
