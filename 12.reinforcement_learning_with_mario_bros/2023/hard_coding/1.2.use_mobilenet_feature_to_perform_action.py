...
from tensorflow.keras.utils import load_img, img_to_array, disable_interactive_logging
...
mobile_net_image_database = Database_Of_Yingshaoxo(database_name="mobile_net_image_data", use_sqlite=False)
...
model_path = disk.join_paths(current_folder, "model.keras")
if not disk.exists(model_path):
    keras_layer = hub.KerasLayer("https://tfhub.dev/google/imagenet/mobilenet_v2_100_224/feature_vector/5", trainable=False)
    model = Sequential([
        keras_layer,
